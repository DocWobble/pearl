...
bool BCacheKey::operator==(const BCacheKey& other) const {
  return generation == other.generation && std::memcmp(job_key.bytes, other.job_key.bytes, 32) == 0 &&
      std::memcmp(hash_b.bytes, other.hash_b.bytes, 32) == 0 &&
      std::memcmp(bound_b.little_endian, other.bound_b.little_endian, 32) == 0 &&
      n == other.n && k == other.k && rank == other.rank && certificate_version == other.certificate_version &&
      std::memcmp(b_noise_seed.bytes, other.b_noise_seed.bytes, 32) == 0 && b_identity == other.b_identity;
}
...
bool DeviceScratch::observe_b(const JobContext& job, const CandidateBatch& candidates) {
  ...
  key.n = job.n; key.k = job.k; key.rank = job.rank;
  ...
  // The replay test uses a null pointer to exercise generation-key invalidation without a
  // CUDA device.  Keep that metadata-only mode out of search: search rejects a null cache.
  ...
  const bool hit = b_cache_valid && b_cache_device != nullptr && b_cache_bytes == bytes && b_cache_key == key;
  if (hit) return true;
  ...
  if (b_cache_device == nullptr && cudaMalloc(&b_cache_device, bytes) != cudaSuccess) {
    b_cache_valid = false;
    return false;
  }
  if (cudaMemcpy(b_cache_device, candidates.b_noised_t, bytes, cudaMemcpyDeviceToDevice) != cudaSuccess) {
    cudaFree(b_cache_device);
    b_cache_device = nullptr;
    b_cache_bytes = 0;
    b_cache_valid = false;
...
void DeviceScratch::release() {
  ...
  winner_queue = nullptr; stream_compute = nullptr; stream_prepare = nullptr;
  buffer_ready[0] = nullptr; buffer_ready[1] = nullptr;
  max_m = max_n = max_k = 0; streams_enabled = false; b_cache_valid = false; b_cache_key = {};
  b_cache_device = nullptr; b_cache_bytes = 0;
}
...
SearchResult search_job(const JobContext& job, const CandidateBatch& candidates, DeviceScratch& scratch) {
  ...
  if (!runtime_enabled() || job.rank != 128 || job.m == 0 || job.n == 0 || job.m % 16 || job.n % 16 || job.k < 16 * job.rank || job.k > 65536 ||
      job.k % job.rank != 0 || candidates.a_noised == nullptr || candidates.b_noised_t == nullptr ||
      !flag_enabled("PEARL_SM120_FUSED") || !flag_enabled("PEARL_SM120_SEARCH_ONLY")) {
    if (!reuse_alloc) scratch.release();
    return result;
  }
...
SearchResult noise_and_search_job(const JobContext& job, const int8_t* a_base,
                                  ...
                                  cudaStream_t stream) {
  SearchResult result{};
  if (!runtime_enabled() || !flag_enabled("PEARL_SM120_FUSED") || !flag_enabled("PEARL_SM120_SEARCH_ONLY") || a_base == nullptr ||
      b_base_t == nullptr || a_rows == nullptr || b_cols == nullptr || candidate_count == 0 ||
      job.rank != 128 || job.m == 0 || job.n == 0 || job.m % 16 || job.n % 16 ||
      job.k < 16 * job.rank || job.k > 65536 || job.k % job.rank != 0) return result;
  ...
  int8_t *a_noise = nullptr, *b_noise = nullptr, *a_noised = nullptr, *b_noised = nullptr;
  const auto cleanup = [&]() {
  ...
  if (cudaMalloc(&a_noise, a_elements) != cudaSuccess || cudaMalloc(&b_noise, b_elements) != cudaSuccess ||
      cudaMalloc(&a_noised, a_elements * candidate_count) != cudaSuccess || cudaMalloc(&b_noised, b_elements) != cudaSuccess) {
    cleanup(); result.metrics.cuda_errors = 1; return result;
  }
  const bool implicit_zero_base = flag_enabled("PEARL_SM120_IMPLICIT_ZERO_BASE");
  if (generate_noise_launch(job.jackpot_key.bytes, job.b_noise_seed.bytes, a_rows, job.m, b_cols, job.n,
                            job.k, job.rank, a_noise, b_noise, prep) != 0 ||
      (implicit_zero_base
           ? repeat_candidates_launch(a_noise, a_noised, a_elements, candidate_count, prep)
           : add_noise_candidates_launch(a_base, a_noise, a_noised, a_elements, candidate_count, prep)) != 0 ||
      (implicit_zero_base
           ? repeat_candidates_launch(b_noise, b_noised, b_elements, 1, prep)
           : add_noise_candidates_launch(b_base_t, b_noise, b_noised, b_elements, 1, prep)) != 0 ||
