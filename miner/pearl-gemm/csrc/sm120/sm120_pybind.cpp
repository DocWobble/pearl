...
void check_cpu_u8_seed(const torch::Tensor& tensor, const char* name) {
  TORCH_CHECK(tensor.device().is_cpu() && tensor.is_contiguous() && tensor.scalar_type() == at::kByte && tensor.numel() == 32,
...
void check_cuda_i32_indices(const torch::Tensor& tensor, const char* name) {
  TORCH_CHECK(tensor.is_cuda() && tensor.is_contiguous() && tensor.scalar_type() == at::kInt,
...
py::dict result_dict(const pearl::sm120::SearchResult& result) {
  ...
  py::list winners;
  const uint64_t copied = result.metrics.winner_count < pearl::sm120::kWinnerQueueCapacity
  ...
  for (uint64_t i = 0; i < copied; ++i) {
    ...
    item["jackpot_hash"] = py::bytes(reinterpret_cast<const char*>(winner.jackpot_hash.bytes), 32);
    winners.append(item);
  }
...
py::dict sm120_search(
    torch::Tensor candidate_a, torch::Tensor b_transposed, torch::Tensor jackpot_key_cpu,
    torch::Tensor share_target_cpu, int64_t m, int64_t n, int64_t k, int64_t generation) {
...
py::dict sm120_noise_search(torch::Tensor candidate_a_base, torch::Tensor b_base_t,
                            torch::Tensor a_noise_seed_cpu, torch::Tensor b_noise_seed_cpu,
                            torch::Tensor a_rows_cuda, torch::Tensor b_cols_cuda,
                            torch::Tensor share_target_cpu, int64_t m, int64_t n, int64_t k,
                            int64_t generation) {
  ...
  TORCH_CHECK(m > 0 && n > 0 && k >= 16 * 128 && m % 16 == 0 && n % 16 == 0 &&
                  k <= 65536 && k % 128 == 0,
  ...
  TORCH_CHECK(batch > 0 && candidate_a_base.numel() == batch * m * k &&
                  b_base_t.numel() == n * k && a_rows_cuda.numel() == m && b_cols_cuda.numel() == n,
              "base operand and index sizes do not match m,n,k");
  pearl::sm120::JobContext job{};
  job.generation = generation;
  job.m = m; job.n = n; job.k = k; job.rank = 128;
  ...
  const auto result = pearl::sm120::noise_and_search_job(
      ...
      reinterpret_cast<const uint32_t*>(b_cols_cuda.data_ptr<int32_t>()), uint32_t(batch),
      reinterpret_cast<uint64_t>(b_base_t.data_ptr<int8_t>()), scratch);
  return result_dict(result);
}

py::tuple sm120_noise(torch::Tensor a_noise_seed_cpu, torch::Tensor b_noise_seed_cpu,
                      torch::Tensor a_rows_cuda, torch::Tensor b_cols_cuda,
  ...
  const auto options = torch::TensorOptions().device(a_rows_cuda.device()).dtype(torch::kInt8);
  auto noise_a = torch::empty({a_rows_cuda.numel(), k}, options);
  auto noise_b_t = torch::empty({b_cols_cuda.numel(), k}, options);
  auto a_seed_cuda = a_noise_seed_cpu.to(a_rows_cuda.device(), torch::kUInt8, false, true);
  auto b_seed_cuda = b_noise_seed_cpu.to(b_cols_cuda.device(), torch::kUInt8, false, true);
  TORCH_CHECK(pearl::sm120::generate_noise_launch(a_seed_cuda.data_ptr<uint8_t>(), b_seed_cuda.data_ptr<uint8_t>(),
                                    reinterpret_cast<const uint32_t*>(a_rows_cuda.data_ptr<int32_t>()), a_rows_cuda.numel(),
                                    reinterpret_cast<const uint32_t*>(b_cols_cuda.data_ptr<int32_t>()), b_cols_cuda.numel(), k, rank,
                                    noise_a.data_ptr<int8_t>(), noise_b_t.data_ptr<int8_t>(), nullptr) == 0,
              "SM120 canonical noise launch failed");
  TORCH_CHECK(cudaDeviceSynchronize() == cudaSuccess, "SM120 canonical noise execution failed");
  return py::make_tuple(noise_a, noise_b_t);
}

torch::Tensor sm120_transcript(torch::Tensor a_noised, torch::Tensor b_noised_t, int64_t k, int64_t rank) {
  check_cuda_i8(a_noised, "a_noised");
  check_cuda_i8(b_noised_t, "b_noised_t");
  TORCH_CHECK(a_noised.numel() == 16 * k && b_noised_t.numel() == 16 * k,
              "transcript probe requires exactly one 16x16 tile");
  TORCH_CHECK(k >= 16 * rank && k <= 65536 && rank == 128 && k % rank == 0,
              "canonical transcript requires rank 128 and K >= 16*rank");
  const auto options = torch::TensorOptions().device(a_noised.device()).dtype(torch::kInt);
  auto transcript = torch::empty({16}, options);
  TORCH_CHECK(pearl::sm120::dense_reference_tile_launch(a_noised.data_ptr<int8_t>(), b_noised_t.data_ptr<int8_t>(),
                                                          k, rank,
                                                          reinterpret_cast<uint32_t*>(transcript.data_ptr<int32_t>())) == 0,
              "SM120 transcript launch failed");
  TORCH_CHECK(cudaDeviceSynchronize() == cudaSuccess, "SM120 transcript execution failed");
  return transcript;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("noise_search", &sm120_noise_search, "Canonical GPU noising followed by exact SM120 search");
  m.def("signal_header", &sm120_signal_header, "Write a validated SM120 winner into Pearl's host signal format");
  m.def("noise", &sm120_noise, "Canonical selected-row/column rank-128 noise generator");
  m.def("transcript", &sm120_transcript, "Exact rank-boundary transcript for one 16x16 tile");
}
